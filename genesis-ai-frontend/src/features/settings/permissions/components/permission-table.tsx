...
interface PermissionTableProps {
  ...
  onAddChild: (permission: PermissionListItem) => void
  onDelete: (permission: PermissionListItem) => void
}

interface TreeNode extends PermissionListItem {
  children: TreeNode[]
  icon?: string | null
  path?: string | null
}

export function PermissionTable({ data, expandAll, onEdit, onAddChild, onDelete }: PermissionTableProps) {
  // 展开状态（记录展开的节点ID）
  ...
  // 渲染图标
  const renderIcon = (iconName?: string | null): ReactElement => {
    if (!iconName) {
      ...
      <Fragment key={node.id}>
        <TableRow>
          ...
          <TableCell>
            ...
            >
              {node.type === 'directory' ? '目录' : node.type === 'menu' ? '菜单' : '功能'}
            </Badge>
          </TableCell>
          ...
          {/* 创建时间 */}
          <TableCell>
            <span className="text-sm text-muted-foreground">{node.created_at}</span>
          </TableCell>
    ...
    <Table>
      <TableHeader>
        <TableRow>
          ...
          <TableHead className="text-center">图标</TableHead>
